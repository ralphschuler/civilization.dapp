...
const deployment = {
  game: getAddress(
    process.env.CIVILIZATION_TESTNET_GAME ??
      "0xfCdB50926c3c6b2CDF3ACE76B13c9383A2DC3199",
  ),
  worldToken: getAddress(
    process.env.CIVILIZATION_TESTNET_WORLD_TOKEN ??
      "0x29147C7BEAd901E8019d7911A7DC404447877C62",
  ),
  worldIdVerifier: getAddress(
    process.env.CIVILIZATION_TESTNET_WORLD_ID_VERIFIER ??
      "0x1A64F89881FD2E38255E62c6D62b68076052DF4b",
  ),
  worldIdLegacyRouter: getAddress(
    process.env.CIVILIZATION_TESTNET_WORLD_ID_LEGACY_ROUTER ??
      process.env.CIVILIZATION_TESTNET_WORLD_ID_VERIFIER ??
      "0x1A64F89881FD2E38255E62c6D62b68076052DF4b",
  ),
...
const keyFile =
  process.env.WORLDCHAIN_TESTNET_KEY_FILE ??
...
const publicClient = createPublicClient({
  ...
  transport: http(worldchainSepolia.rpcUrls.default.http[0]),
});
const { receiverPrivateKey } = JSON.parse(await readFile(keyFile, "utf8"));
if (!receiverPrivateKey)
  throw new Error("testnet key file lacks receiverPrivateKey");
...
async function writeContract(request) {
  ...
  if (receipt.status !== "success")
    throw new Error(`testnet transaction failed: ${hash}`);
  return hash;
}
...
const [
  ...
  publicClient.getCode({ address: deployment.game }),
  publicClient.readContract({
    address: deployment.game,
    abi: gameAbi,
    functionName: "worldIdVerifier",
  }),
  publicClient.readContract({
    address: deployment.game,
    abi: gameAbi,
    functionName: "worldIdLegacyRouter",
  }),
  publicClient.readContract({
    address: deployment.game,
    abi: gameAbi,
    functionName: "worldToken",
  }),
  publicClient.readContract({
    address: deployment.game,
    abi: gameAbi,
    functionName: "boostTreasury",
  }),
  readState(),
...
if (
  ...
  !code ||
  verifier !== deployment.worldIdVerifier ||
  legacyRouter !== deployment.worldIdLegacyRouter ||
  token !== deployment.worldToken ||
  treasury !== deployment.treasury
) {
  throw new Error(
    "deployed testnet contract configuration does not match the expected manifest",
  );
}
...
if (process.env.CONFIRM_TESTNET_E2E !== "yes")
  throw new Error(
    "set CONFIRM_TESTNET_E2E=yes to submit testnet game transactions",
  );
...
if (!current.registered) {
  ...
  const nullifierHash = BigInt(
    keccak256(stringToHex(`civilization-testnet:${wallet.toLowerCase()}`)),
  );
  report.transactions.push({
    step: "mock_world_id_registration",
...
if (
  current.construction.pending &&
  current.construction.completesAt <= blockBeforeActions.timestamp
) {
  report.transactions.push({
    step: "complete_upgrade",
    hash: await writeContract({
      address: deployment.game,
      abi: gameAbi,
...
const remaining =
  current.construction.pending &&
  current.construction.completesAt > latestBlock.timestamp
    ? current.construction.completesAt - latestBlock.timestamp
    : 0n;
...
if (boostHours > 0n) {
  ...
  report.transactions.push({
    step: "mint_mock_wld",
    hash: await writeContract({
      address: deployment.worldToken,
      abi: tokenAbi,
      functionName: "mint",
      args: [account.address, wld],
    }),
  });
  report.transactions.push({
    step: "approve_mock_wld",
    hash: await writeContract({
      address: deployment.worldToken,
      abi: tokenAbi,
      functionName: "approve",
      args: [deployment.game, wld],
    }),
  });
  report.transactions.push({
    step: "boost_construction",
    hash: await writeContract({
      address: deployment.game,
      abi: gameAbi,
