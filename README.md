...
Server-only production configuration includes `WALLET_AUTH_URL`, `DATABASE_URL` or the documented `PG*` values. WalletAuth additionally requires a 32+ character `WALLET_AUTH_RATE_LIMIT_SECRET` and `WALLET_AUTH_TRUSTED_PROXY_HOPS`. At the ingress, overwrite (never append to an untrusted value) `X-Forwarded-For` and configure the exact number of proxy addresses appended to that header; without that trusted deployment contract requests fall into one conservative anonymous bucket. PostgreSQL stores only short-lived, one-time WalletAuth/SIWE challenges and HMAC-hashed rate-limit identities. `WORLD_APP_ID`, `CIVILIZATION_CHAIN_ID`, game contract, and WLD token addresses are read by Next at runtime and serialized into client props, so the container image contains no deployment-specific App ID. Production accepts only World Chain `480` and the configured canonical WLD address; malformed or mismatched values disable the transaction UI. Never commit secrets.

Before `master` can publish, the candidate production image runs `pnpm gate:worldchain:production` against its configured HTTPS World Chain RPC. The gate is read-only and emits JSON; it requires the exact reviewed V2 implementation address and runtime codehash in `CIVILIZATION_WORLDCHAIN_V2_IMPLEMENTATION_ADDRESS` and `CIVILIZATION_WORLDCHAIN_V2_IMPLEMENTATION_CODEHASH`. Configure those as protected release variables and the RPC URL (plus the rate-limit secret needed to validate the complete production configuration) as protected release secrets. The active proxy is intentionally V1 today, so leave the V2 values unset until a reviewed V2 runtime exists: master delivery must remain blocked rather than publishing an image with incompatible client assumptions. Development does not consume these production-only values.

The `wallet_auth_metrics` table exposes minute-bucketed aggregate counters (`nonce_issued`, `nonce_rate_limited`, `nonce_challenge_capacity_limited`, `verify_rate_limited`, and `verify_succeeded`) for monitoring. It deliberately contains no client identity, wallet address, nonce, or signature.
...
The cosmetic village appearance preference is private to that WalletAuth session's checksum wallet and is available only as `classic` (default), `dusk`, or `dawn`. `/api/village-appearance` is authenticated, `no-store`, and takes no wallet identifier; its server allowlist and database constraint reject every other value. A failed, malformed, unavailable, or expired-session response renders `classic` without exposing another wallet's preference.
