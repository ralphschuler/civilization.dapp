...
The cosmetic village appearance preference is private to that WalletAuth session's checksum wallet and is available only as `classic` (default) or `dusk`. `/api/village-appearance` is authenticated, `no-store`, and takes no wallet identifier; its server allowlist and database constraint reject every other value. A failed, malformed, unavailable, or expired-session response renders `classic` without exposing another wallet's preference.
