...
`upgrade` pays only its on-chain cost and queues one construction. `completeUpgrade` must be called after the exact minute curve `1.1 * 1.569772144168414^(level - 1) + 0.9`: the contract stores the factor as 1e18 WAD, rounds the final seconds upward, and caps any duration at 365 days (level 1 is 120 seconds). `boostConstruction(hoursToBoost)` charges exactly one WLD per requested full hour, reduces a pending timer by that many hours, and rejects a boost beyond completion. Its exact receipt checks split each payment 50/50: floor-half WLD is credited to the configured `CivilizationBuybackVault`, while any odd-wei remainder goes to the developer revenue splitter; the game never holds WLD. A splitter payout failure is best-effort and cannot undo the boost or make vault WLD claimable. The splitter supports two to ten recipients, checkpoints each schedule, and once per 30 days permissionlessly transfers the current recipients' WLD shares in the payout transaction. Removed recipients retain checkpointed claims for the permissionless `release(recipient)` fallback. Any player action attempts that processing with a bounded gas budget sized for the ten-recipient maximum, without allowing a payout failure to block gameplay; missed periods advance in constant time. `prestige` requires Townhall 30 with no construction pending; it resets village state and adds a permanent 10% production multiplier per prestige. It preserves the player's CGOLD.

## Source-only buyback vault

`CivilizationBuybackVault` has immutable WLD token, Game proxy, and timelock bindings. Only the configured Game may credit exact-receipt WLD; direct transfers are not spendable credits. The timelock explicitly configures one code-bearing V3-compatible router, one WLD/CGOLD pool, fee, threshold, batch limit, TWAP window, slippage limit, and pause state. Permissionless `execute` uses `observe([window, 0])` to derive a mean tick and fails closed—without a spot quote—when observations are unavailable. The route must spend exactly the credited batch and satisfy the TWAP-derived minimum, then all received CGOLD is immediately burned through the Game's holder-authorized `burn(uint256)`. There is no withdrawal, arbitrary call, public mint, venue address, price claim, or return promise. Deferred pause/threshold/deadline/observation/router conditions emit events without touching splitter claimability.
