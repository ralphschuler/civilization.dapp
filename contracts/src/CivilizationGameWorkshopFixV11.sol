...
/// @title CivilizationGameWorkshopFixV11
/// @notice Minimal V1.1 implementation: Workshop 0 -> 1 does not require CGOLD.
...
contract CivilizationGameWorkshopFixV11 is Initializable {
    ...
    event Prestiged(
        address indexed player,
        uint256 prestigeCount,
        uint256 productionMultiplierBps
    );
    event ConstructionBoosted(
        address indexed player,
        uint256 hoursBoosted,
        uint256 wldPaid,
        uint64 completesAt
    );
    ...
    function initialize(InitConfig calldata config) external initializer {
        if (
            ...
            config.revenueSplitter == address(0) ||
        ...
        _validateRevenueSplitter(
            config.revenueSplitter,
            config.worldToken,
            config.timelock
        );
        ...
        $.worldIdCredentialGenesisIssuedAtMin = config
            .credentialGenesisIssuedAtMin;
        $.worldIdLegacyRouter = IWorldIDLegacyRouter(
            config.worldIdLegacyRouter
        );
        $.worldIdLegacyExternalNullifier = _hashToField(
            abi.encodePacked(
                _hashToField(bytes(config.worldIdLegacyAppId)),
                config.worldIdLegacyActionId
            )
        );
    ...
    function totalSupply() external view returns (uint256) {
        return _game().totalSupply;
    }
    function balanceOf(address account) external view returns (uint256) {
        return _game().balanceOf[account];
    }
    function allowance(
        address owner,
        address spender
    ) external view returns (uint256) {
        return _game().allowance[owner][spender];
    }
    function nullifierOwner(uint256 value) external view returns (address) {
        return _game().nullifierOwner[value];
    }
    function worldIdVerifier() external view returns (IWorldIDVerifier) {
        return _game().worldIdVerifier;
    }
    function worldIdAction() external view returns (uint256) {
        return _game().worldIdAction;
    }
    function worldIdRpId() external view returns (uint64) {
        return _game().worldIdRpId;
    }
    function worldIdLegacyRouter()
        external
        view
        returns (IWorldIDLegacyRouter)
    {
        return _game().worldIdLegacyRouter;
    }
    function worldIdLegacyExternalNullifier() external view returns (uint256) {
        return _game().worldIdLegacyExternalNullifier;
    }
    function worldToken() external view returns (address) {
        return _game().worldToken;
    }
    ...
    function registerWorldId(
        ...
        GameStorage storage $ = _game();
        if (issuerSchemaId != $.worldIdIssuerSchemaId)
            revert UnexpectedWorldIdCredential();
    ...
    function _requireAvailableRegistration(
        uint256 nullifierHash,
        uint256 signalHash
    ) private view {
        if (_game().players[msg.sender].registered) revert AlreadyRegistered();
        if (_game().nullifierOwner[nullifierHash] != address(0))
            revert NullifierAlreadyUsed();
        if (signalHash != _hashToField(abi.encodePacked(msg.sender)))
            revert InvalidWorldIdConfiguration();
    }
    ...
    function claim() external onlyRegistered {
        ...
        (claimed.wood, player.stored.wood, player.field.wood) = _claimOne(
            player.stored.wood,
            player.field.wood,
            capacity
        );
        (claimed.clay, player.stored.clay, player.field.clay) = _claimOne(
            player.stored.clay,
            player.field.clay,
            capacity
        );
        (claimed.stone, player.stored.stone, player.field.stone) = _claimOne(
            player.stored.stone,
            player.field.stone,
            capacity
        );
    ...
    function startRaid(
        ...
        if (attacker.pendingRaid.defender != address(0))
            revert RaidAlreadyPending();
        _accrue(attacker);
        if (
            spear > attacker.troops.spear ||
            archer > attacker.troops.archer ||
            rider > attacker.troops.rider
        ) revert InsufficientTroops();
        attacker.troops.spear -= spear;
        attacker.troops.archer -= archer;
        attacker.troops.rider -= rider;
        attacker.pendingRaid = Raid(
            defender,
            uint64(block.timestamp + RAID_MARCH_DURATION),
            spear,
            archer,
            rider
        );
        emit RaidStarted(
            msg.sender,
            defender,
            attacker.pendingRaid.arrivesAt,
            spear,
            archer,
            rider
        );
    ...
    function resolveRaid() external onlyRegistered {
        ...
        if (block.timestamp < raid.arrivesAt)
            revert RaidNotArrived(raid.arrivesAt);
        ...
        (
            ...
            uint256 archerLost,
            uint256 riderLost
        ) = _casualties(raid.spear, raid.archer, raid.rider, won ? 8 : 38);
        ...
        Resources memory stolen =
            won
                ? _loot(
                    msg.sender,
                    attacker,
                    defender,
                    (raid.spear + raid.archer + raid.rider) * 18
                )
                : Resources(0, 0, 0, 0);
        emit RaidResolved(
            msg.sender,
    ...
    function playerState(
        ...
        returns (
            ...
            Resources memory stored,
            Resources memory field,
            Buildings memory buildings,
            Troops memory troops,
            Raid memory pendingRaid,
        ...
        )
    {
        ...
        return (
            player.registered,
            player.lastAccruedAt,
            _claimAvailableAt(player),
            player.stored,
            player.field,
            player.buildings,
            player.troops,
    ...
    function previewAccrual(
        ...
        view
        returns (
            Resources memory wholeField,
            Resources memory fractionalRemainder,
            uint256 fractionScale,
            uint64 asOf
        )
    {
        Player storage player = _game().players[account];
        ...
        if (player.registered) {
            (wholeField, fractionalRemainder) = _accruedField(
                wholeField,
                fractionalRemainder,
                player.buildings,
                player.prestigeCount,
    ...
    function buildDuration(
        ...
        if (nextLevel == 0 || nextLevel > MAX_BUILDING_LEVEL)
            revert InvalidBuildingLevel();
        uint256 wad = 1e18;
    ...
    function transferFrom(
        ...
        if (permitted != type(uint256).max) {
            ...
            emit Approval(
                from,
                msg.sender,
                _game().allowance[from][msg.sender]
            );
        }
    ...
    modifier onlyRegistered() {
        if (!_game().players[msg.sender].registered) revert Unregistered();
        _;
    }
    ...
    function _accrueUntil(Player storage player, uint256 to) private {
        ...
        (player.field, player.accrualRemainder) = _accruedField(
            player.field,
            player.accrualRemainder,
            player.buildings,
            player.prestigeCount,
    ...
    function _accrueForCompletion(
        Player storage player,
        uint64 completesAt
    ) private returns (uint256 cappedEnd) {
        cappedEnd = _min(
            block.timestamp,
            uint256(player.lastAccruedAt) + MAX_OFFLINE_SECONDS
        );
    ...
    function _accruedField(
        Resources memory field,
        Resources memory remainder,
        Buildings memory buildings,
        uint256 prestigeCount,
        uint64 from,
        uint256 to
    ) private pure returns (Resources memory, Resources memory) {
        ...
        (field.wood, remainder.wood) = _accrueOne(
            field.wood,
            remainder.wood,
            elapsed,
            300 * buildings.timber * multiplier,
            capacity
        );
        (field.clay, remainder.clay) = _accrueOne(
            field.clay,
            remainder.clay,
            elapsed,
            270 * buildings.claypit * multiplier,
            capacity
        );
        (field.stone, remainder.stone) = _accrueOne(
            field.stone,
            remainder.stone,
            elapsed,
            240 * buildings.quarry * multiplier,
            capacity
        );
        (field.gold, remainder.gold) = _accrueOne(
            field.gold,
            remainder.gold,
            elapsed,
            12 * buildings.goldmine * multiplier,
            capacity
        );
        return (field, remainder);
    }

    function _cappedElapsed(
        uint64 from,
        uint256 to
    ) private pure returns (uint256 elapsed) {
        elapsed = to - from;
    ...
    function _loot(
        address attackerAccount,
        Player storage attacker,
        Player storage defender,
        uint256 transportCapacity
    ) private returns (Resources memory stolen) {
        uint256 capacity = _capacity(attacker.buildings.warehouse);
        uint256 remaining = transportCapacity;
        uint256 woodRoom =
            capacity > attacker.stored.wood
                ? capacity - attacker.stored.wood
                : 0;
        uint256 clayRoom =
            capacity > attacker.stored.clay
                ? capacity - attacker.stored.clay
                : 0;
        uint256 stoneRoom =
            capacity > attacker.stored.stone
                ? capacity - attacker.stored.stone
                : 0;
        stolen.wood = _min(defender.field.wood, _min(remaining, woodRoom));
        remaining -= stolen.wood;
        defender.field.wood -= stolen.wood;
        attacker.stored.wood += stolen.wood;
        stolen.clay = _min(defender.field.clay, _min(remaining, clayRoom));
        remaining -= stolen.clay;
        defender.field.clay -= stolen.clay;
        attacker.stored.clay += stolen.clay;
        stolen.stone = _min(defender.field.stone, _min(remaining, stoneRoom));
        remaining -= stolen.stone;
        defender.field.stone -= stolen.stone;
        attacker.stored.stone += stolen.stone;
        (stolen.gold, ) = _take(defender.field.gold, remaining);
        defender.field.gold -= stolen.gold;
        if (stolen.gold != 0)
            _mintGold(attackerAccount, stolen.gold * GOLD_UNIT);
    }

    function _buildingCost(
        Buildings storage b,
        Building building
    ) private view returns (Resources memory price) {
        uint256 level = _buildingLevel(b, building);
        uint256 factor;
        Resources memory base;
        if (building == Building.Townhall) {
            base = Resources(280, 260, 240, 0);
            factor = 160;
        } else if (building == Building.Timber) {
            base = Resources(35, 20, 15, 0);
            factor = 146;
        } else if (building == Building.Claypit) {
            base = Resources(25, 40, 20, 0);
            factor = 147;
        } else if (building == Building.Quarry) {
            base = Resources(30, 25, 45, 0);
            factor = 148;
        } else if (building == Building.Warehouse) {
            base = Resources(45, 45, 35, 0);
            factor = 152;
        } else if (building == Building.Workshop) {
            base = Resources(90, 110, 105, 15);
            factor = 160;
        } else if (building == Building.Goldmine) {
            base = Resources(130, 120, 150, 0);
            factor = 166;
        } else {
            base = Resources(125, 145, 105, 25);
            factor = 162;
        }
        for (uint256 i; i < level; ++i) {
            base.wood = _ceilMul(base.wood, factor);
            base.clay = _ceilMul(base.clay, factor);
    ...
    function _troopCost(
        ...
        if (troop == Troop.Spear)
            return Resources(22 * amount, 16 * amount, 8 * amount, amount);
        if (troop == Troop.Archer)
            return Resources(30 * amount, 22 * amount, 12 * amount, 2 * amount);
        return Resources(45 * amount, 35 * amount, 24 * amount, 4 * amount);
    }

    function _requireBuildingRequirements(
        Buildings storage b,
        Building building
    ) private view {
        ...
        if (
            ...
            (b.timber < next ||
                b.claypit < next ||
                b.quarry < next ||
                (next >= 3 && b.warehouse < next - 1) ||
                (next >= 5 && b.workshop < next - 3))
        ) revert MissingBuildingRequirement();
        if (building == Building.Warehouse && b.townhall < 1)
            revert MissingBuildingRequirement();
        if (
            building == Building.Workshop &&
            (b.townhall < 2 || b.timber < 2 || b.claypit < 2 || b.quarry < 2)
        ) revert MissingBuildingRequirement();
        if (building == Building.Goldmine && (b.townhall < 4 || b.workshop < 2))
            revert MissingBuildingRequirement();
        if (building == Building.Barracks && (b.townhall < 3 || b.workshop < 1))
            revert MissingBuildingRequirement();
    }

    function _requireTroopRequirements(
        Buildings storage b,
        Troop troop
    ) private view {
        if (
            b.barracks < 1 ||
            (troop == Troop.Archer && b.barracks < 2) ||
            (troop == Troop.Rider && (b.barracks < 3 || b.workshop < 2))
        ) revert MissingBuildingRequirement();
    }
    function _claimAvailableAt(
        Player storage player
    ) private view returns (uint64) {
        return
            player.lastClaimedAt == 0
                ? 0
                : uint64(uint256(player.lastClaimedAt) + CLAIM_COOLDOWN);
    }
    function _validateRevenueSplitter(
        address splitter,
        address expectedToken,
        address expectedTimelock
    ) private view {
        if (splitter.code.length == 0) revert InvalidRevenueSplitter(splitter);
        try ICivilizationRevenueSplitter(splitter).token() returns (
            IERC20 token_
        ) {
        ...
        try ICivilizationRevenueSplitter(splitter).timelock() returns (
            address timelock_
        ) {
    ...
    function _tryMonthlyPayout() private {
        ...
        (bool success, bytes memory reason) = splitter.call{
            gas: MONTHLY_PAYOUT_CALL_GAS
        }(
            abi.encodeCall(
                ICivilizationRevenueSplitter.processMonthlyPayout,
                ()
            )
        );
        if (!success) emit MonthlyPayoutDeferred(splitter, keccak256(reason));
    }
    function _productionMultiplier(
        uint256 prestigeCount
    ) private pure returns (uint256) {
        return BASIS_POINTS + prestigeCount * PRESTIGE_BONUS_BPS;
    }
    function _spend(
        Player storage player,
        address account,
        Resources memory price
    ) private {
        if (
        ...
        if (_game().balanceOf[account] < goldCost)
    ...
    function _mintGold(address account, uint256 value) private {
        _game().totalSupply += value;
        _game().balanceOf[account] += value;
        emit Transfer(address(0), account, value);
    }
    function _burnGold(address account, uint256 value) private {
        if (_game().balanceOf[account] < value)
            revert InsufficientGoldBalance();
        _game().balanceOf[account] -= value;
        _game().totalSupply -= value;
        emit Transfer(account, address(0), value);
    }
    function _transferGold(address from, address to, uint256 value) private {
        if (to == address(0)) revert ZeroAddress();
        if (_game().balanceOf[from] < value) revert InsufficientGoldBalance();
        _game().balanceOf[from] -= value;
        _game().balanceOf[to] += value;
        emit Transfer(from, to, value);
    }
    function _defense(Player storage p) private view returns (uint256) {
        uint256 troopPower =
            p.troops.spear * 10 + p.troops.archer * 17 + p.troops.rider * 31;
        return _max(1, (troopPower * 65) / 100 + p.buildings.townhall * 20);
    }
    function _casualties(
        uint256 spear,
        uint256 archer,
        uint256 rider,
        uint256 rate
        ...
        return (
            _ceilPercent(spear, rate),
            _ceilPercent(archer, rate),
            _ceilPercent(rider, rate)
        );
    }
    function _capacity(
        uint256 warehouse
        ...
        result = 500;
        for (uint256 i = 1; i < warehouse; ++i) result = (result * 17 + 5) / 10;
    }
    function _claimOne(
        uint256 stored,
        uint256 field,
        ...
        private
        pure
        returns (uint256 claimed, uint256 nextStored, uint256 nextField)
    {
        claimed = _min(field, capacity > stored ? capacity - stored : 0);
        return (claimed, stored + claimed, field - claimed);
    }
    function _take(
        uint256 available,
        uint256 wanted
    ) private pure returns (uint256 amount, uint256 remaining) {
        amount = _min(available, wanted);
        return (amount, wanted - amount);
    }
    function _incrementBuilding(
        Buildings storage b,
        Building building
    ) private returns (uint256) {
        if (building == Building.Townhall) return ++b.townhall;
        if (building == Building.Timber) return ++b.timber;
        if (building == Building.Claypit) return ++b.claypit;
        if (building == Building.Quarry) return ++b.quarry;
        if (building == Building.Warehouse) return ++b.warehouse;
    ...
    function _ceilMul(
        ...
        return (value * factor + 99) / 100;
    }
    function _ceilPercent(
        uint256 value,
        uint256 percent
    ) private pure returns (uint256) {
