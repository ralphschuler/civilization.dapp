...
test("contract status identifies the unaudited World Chain mainnet deployment without settlement", () => {
  assert.equal(
    CONTRACT_STATUS.release,
    "mainnet_wallet_registration_deployed_no_settlement",
  );
  assert.equal(
    CONTRACT_STATUS.deployment,
    "worldchain_mainnet_wallet_registration_deployed",
  );
...
test("Solidity sources remain free of deployment configuration", async () => {
  for (const contract of CONTRACT_STATUS.contracts) {
    const source = await readFile(
      new URL(`../${contract.source}`, import.meta.url),
      "utf8",
    );
    ...
    for (const line of source.split("\n")) {
      if (!/bytes32\s+(?:private\s+)?constant/.test(line))
        assert.doesNotMatch(line, /0x[a-fA-F0-9]{40}(?![a-fA-F0-9])/);
    }
  }
});
