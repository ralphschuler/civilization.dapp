...
import { consumeWalletLoginTicket, hashWalletLoginTicket, WALLET_LOGIN_TICKET_TTL_MS } from '../src/lib/wallet-login-ticket.js';
...
test('ticket hashing is deterministic, one-way shaped, and ticket TTL is at most one minute', () => {
  ...
  assert.ok(WALLET_LOGIN_TICKET_TTL_MS > 0 && WALLET_LOGIN_TICKET_TTL_MS <= 60_000);
});
...
test('a login ticket is minted only after verified SIWE success', async () => {
  ...
  const mintWalletLoginTicket = async (address) => {
    ...
    return { ticket: 'B'.repeat(43), loginId: '11111111-1111-4111-8111-111111111111' };
  };
  const success = await verifyAndMintWalletLoginTicket({}, {
    verifyWalletAuthRequest: async () => ({ kind: 'success', address: '0x52908400098527886E0F7030069857D2E4169EE7' }),
  ...
  for (const kind of ['malformed', 'invalid_nonce', 'verification_failed']) {
    assert.deepEqual(await verifyAndMintWalletLoginTicket({}, {
...
test('malformed ticket does not query the database and a valid row is bound to database wallet and login ID', async () => {
  let calls = 0;
  const query = async () => { calls += 1; return { rowCount: 0, rows: [] }; };
  assert.equal(await consumeWalletLoginTicket('not-a-ticket', query), null);
  assert.equal(calls, 0);

  let sql;
  const result = await consumeWalletLoginTicket('C'.repeat(43), async (issuedSql, params) => {
    sql = issuedSql;
...
test('malformed mint output is rejected without allowing it to override verified identity', async () => {
  await assert.rejects(verifyAndMintWalletLoginTicket({}, {
    verifyWalletAuthRequest: async () => ({ kind: 'success', address: '0x52908400098527886E0F7030069857D2E4169EE7' }),
    mintWalletLoginTicket: async () => ({ ticket: 'F'.repeat(43), loginId: '11111111-1111-4111-8111-111111111111', address: '0x0000000000000000000000000000000000000001' }),
