...
test("public RP request accepts only fixed play action and checksum-normalizable wallet signals", () => {
  assert.deepEqual(
    ...
    { kind: "success", signal: wallet },
  );
  assert.deepEqual(
    validateRpSignatureRequest(
      { action: "other", signal: wallet },
      { action: "play" },
    ),
    { kind: "invalid_action" },
  );
  for (const body of [
    ...
    "bad",
    { action: "play" },
    { action: "play", signal: "not-an-address" },
  ]) {
    assert.notEqual(
      validateRpSignatureRequest(body, { action: "play" }).kind,
      "success",
    );
  }
});
...
test("public RP route uses a bounded JSON reader and has no Auth.js, cookies, logs, or mutable signing context", async () => {
  const [route, contract] = await Promise.all([
    readFile(
      new URL("../src/app/api/rp-signature/route.ts", import.meta.url),
      "utf8",
    ),
    readFile(
  ...
  assert.doesNotMatch(
    route,
    /from ["']\@\/auth["']|\bauth\s*\(|req\.json\(\)|console\.|Set-Cookie|cookie|ticket|wallet-auth\/verify/,
  );
  assert.match(
    contract,
    /signalHash != _hashToField\(abi\.encodePacked\(msg\.sender\)\)/,
  );
