...
test("buy quote retains a 1.5 percent CGOLD sink", () => {
  const quote = quoteCgoldWldTrade({ side: "buy", amount: "100000000000000000000" });
...
test("sell quote retains a 1.5 percent WLD sink", () => {
  const quote = quoteCgoldWldTrade({ side: "sell", amount: "100000000000000000000" });
...
test("quotes reject invalid sides and decimal amounts", () => {
  assert.throws(() => quoteCgoldWldTrade({ side: "buy", amount: "1.2" }), /invalid_amount/);
  assert.throws(() => quoteCgoldWldTrade({ side: "swap", amount: "1" }), /invalid_side/);
});
