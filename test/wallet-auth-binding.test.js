...
test('SIWE binding accepts the actual MiniKit 2 parser output', () => {
  ...
  assert.equal(hasValidSiweBinding({
...
test('SIWE binding accepts configured host or HTTPS origin domains only', () => {
  ...
  assert.equal(hasValidSiweBinding(binding({ domain: authUrl }), authUrl), true);
  ...
  for (const invalid of [
    ...
    assert.equal(hasValidSiweBinding(binding(invalid), authUrl), false, String(invalid.chainId ?? invalid.domain ?? invalid.uri ?? invalid.version));
  }
});
