...
const cost = (wood = 0, clay = 0, stone = 0, gold = 0) => ({
  wood,
  clay,
  stone,
  gold,
...
export const BUILDINGS = {
  townhall: {
    label: "Rathaus",
    icon: "R",
    detail: "Schaltet stärkere Ausbauten frei.",
    base: cost(55, 65, 75, 0),
    factor: 1.58,
    requires: (level) => {
      if (level === 1) return [];
      const requirements = ["timber", "claypit", "quarry"].map((id) => ({
        id,
        level,
      }));
  ...
  warehouse: {
    label: "Speicher",
    icon: "SP",
    detail: "Erhöht Kapazität aller Rohstoffe.",
    base: cost(45, 45, 35),
    factor: 1.52,
  ...
  workshop: {
    label: "Werkstatt",
    icon: "W",
    detail: "Voraussetzung für Gold und Einheiten.",
    base: cost(90, 110, 105, 15),
    factor: 1.6,
    requires: () => [
      { id: "townhall", level: 2 },
      { id: "timber", level: 2 },
  ...
  goldmine: {
    label: "Goldschacht",
    icon: "G",
    detail: "Erzeugt Gold für Ausbildung.",
    base: cost(130, 120, 150, 0),
    factor: 1.66,
    produces: { gold: 0.13 },
    requires: () => [
      { id: "townhall", level: 4 },
  ...
  barracks: {
    label: "Kaserne",
    icon: "K",
    detail: "Bildet Truppen aus.",
    base: cost(125, 145, 105, 25),
    factor: 1.62,
    requires: () => [
      { id: "townhall", level: 3 },
      { id: "workshop", level: 1 },
...
export const TROOPS = {
  spear: {
    ...
    attack: 10,
    cost: cost(22, 16, 8, 1),
    requires: [{ id: "barracks", level: 1 }],
  },
  archer: {
    label: "Bogenschütze",
    icon: "B",
    attack: 17,
    cost: cost(30, 22, 12, 2),
    requires: [
      { id: "barracks", level: 2 },
      { id: "workshop", level: 1 },
    ],
...
const target = (id, name, defense, unclaimed) => ({
  id,
  name,
  defense,
  unclaimed,
...
export function createInitialState(now = Date.now()) {
  return {
    ...
    targets: DEMO_TARGETS.map((item) => ({
      ...item,
      unclaimed: { ...item.unclaimed },
      initialUnclaimed: { ...item.initialUnclaimed },
    })),
    raids: 0,
    lastRaid: null,
    gatherAvailableAt: 0,
...
export function getProduction(state) {
  ...
  Object.entries(BUILDINGS).forEach(([id, building]) => {
    ...
    Object.entries(building.produces).forEach(([resource, rate]) => {
      production[resource] += rate * state.buildings[id];
    });
  });
  return production;
}
...
export function settle(state, now = Date.now()) {
  ...
  Object.keys(RESOURCE_DEFS).forEach((resource) => {
    state.unclaimed[resource] = Math.min(
      capacity,
      state.unclaimed[resource] + production[resource] * elapsed,
    );
  });
  state.last = now;
  return state;
...
export function getRequirements(state, id) {
  const building = BUILDINGS[id];
  return (building.requires?.((state.buildings[id] || 0) + 1) || []).filter(
    ({ id: required, level }) => (state.buildings[required] || 0) < level,
  );
}

export function canPay(resources, required) {
  return Object.keys(RESOURCE_DEFS).every(
    (resource) => resources[resource] >= required[resource],
  );
}
...
export function upgradeBuilding(state, id, now = Date.now()) {
  ...
  if (missingRequirements.length)
    return { ok: false, reason: "requirements", missingRequirements };
  if (!canPay(state.resources, required))
    return { ok: false, reason: "resources", required };
...
export function gather(state, now = Date.now()) {
  ...
  Object.keys(RESOURCE_DEFS).forEach((resource) => {
    collected[resource] = Math.min(
      state.unclaimed[resource],
      Math.max(0, capacity - state.resources[resource]),
    );
...
export function startGathering(state, now = Date.now()) {
  if (now < (state.gatherAvailableAt || 0))
...
export function swapInternal(state, from, to, amount, now = Date.now()) {
  ...
  if (
    !INTERNAL_VALUES[from] ||
    !INTERNAL_VALUES[to] ||
    from === to ||
    !safeAmount
  )
    return { ok: false, reason: "market" };
  if (state.resources[from] < safeAmount)
    return { ok: false, reason: "resources" };
  const output = Math.floor(
    (safeAmount * INTERNAL_VALUES[from]) / INTERNAL_VALUES[to],
  );
  const received = Math.min(
    output,
    Math.max(0, getCapacity(state) - state.resources[to]),
  );
...
export function trainTroop(state, id, amount = 1, now = Date.now()) {
  ...
  const missingRequirements = troop.requires.filter(
    ({ id: required, level }) => state.buildings[required] < level,
  );
  const required = Object.fromEntries(
    Object.entries(troop.cost).map(([resource, value]) => [
      resource,
      value * safeAmount,
    ]),
  );
...
function loot(targetLoot, capacity) {
  const total = Object.values(targetLoot).reduce(
  ...
  Object.keys(RESOURCE_DEFS).forEach((resource) => {
    result[resource] = Math.min(
      targetLoot[resource],
      Math.floor((targetLoot[resource] / total) * capacity),
    );
  });
  return result;
}
...
export function sendRaid(state, targetId, selected, now = Date.now()) {
  ...
  const army = Object.fromEntries(
    Object.keys(TROOPS).map((id) => [
      id,
      Math.max(0, Math.floor(Number(selected[id]) || 0)),
    ]),
  );
  const total = Object.values(army).reduce((sum, value) => sum + value, 0);
  if (!target || !total) return { ok: false, reason: "army" };
  if (Object.keys(army).some((id) => army[id] > state.troops[id]))
    return { ok: false, reason: "availability" };
  const attack = Object.entries(army).reduce(
    (sum, [id, amount]) => sum + TROOPS[id].attack * amount,
    0,
  );
  ...
  const casualties = Object.fromEntries(
    Object.keys(TROOPS).map((id) => [
      id,
      Math.min(army[id], Math.ceil(army[id] * casualtyRate)),
    ]),
  );
  Object.keys(TROOPS).forEach((id) => {
    state.troops[id] -= casualties[id];
  });
  ...
  Object.keys(RESOURCE_DEFS).forEach((resource) => {
    ...
    state.resources[resource] = Math.min(
      getCapacity(state),
      state.resources[resource] + stolen[resource],
    );
  });
  state.raids += 1;
  state.lastRaid = {
...
export function resolveRaidMarch(state, now = Date.now()) {
  if (!state.pendingRaid || now < state.pendingRaid.arrivesAt)
    return { ok: false, reason: "march" };
