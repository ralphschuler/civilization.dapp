...
export async function verifyAndMintWalletLoginTicket(body, dependencies = {}) {
  const verify = dependencies.verifyWalletAuthRequest ?? verifyWalletAuthRequest;
  ...
  if (!minted || typeof minted !== 'object' || Array.isArray(minted)
  ...
  if (typeof ticket !== 'string' || !/^[A-Za-z0-9_-]{43}$/.test(ticket)
    ...
    || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(loginId)) {
    throw new Error('invalid_wallet_login_ticket');
  }
