...
export function validateRpSignatureRequest(body, { action }) {
  if (!body || typeof body !== 'object' || Array.isArray(body)) return { kind: 'invalid_payload' };
  if (body.action !== action) return { kind: 'invalid_action' };
  if (typeof body.signal !== 'string' || !isAddress(body.signal)) return { kind: 'invalid_signal' };
